...
export default defineConfig(({ command, mode, ssrBuild }) => {
  if (!isWpBlocks) {
            ...
            handler: (server) => {
              ...
              if (isHost) {
                setTimeout(() => {
                  ...
                  templateConfig.statistics.enable
                    ? logger(`_DEV_DONE`)
                    : logger(`_DEV_DONE_STAT`);
                }, 1000);
              } else {
                logger(
                  `_DEV_HOST_ADDRESS`,
              ...
              assetFileNames: (asset) => {
                let getPath =
                  ...
                  asset.names.length > 0
                    ? asset.originalFileNames[0].replace(
                        `/${asset.names[0]}`,
                        ''
                      )
                    : '';
    ...
    return {
      ...
      root: path.join(
        __dirname,
        'src/components/wordpress/fls-theme/components/blocks'
      ),
      server: {
        watch: {
          ignored: ['**/*', '!fls-theme/components/blocks/**']
        }
      },
      plugins: [
        // Робота з стилями
      ...
      build: {
        outDir: path.join(
          __dirname,
          'src/components/wordpress/fls-theme/components/blocks/admin/dist'
        ),
        emptyOutDir: true,
        cssMinify: false,
        rollupOptions: {
          input: [
            ...globSync(
              './src/components/wordpress/fls-theme/components/blocks/**/*.js',
              { ignore: ['**/dist/**'] }
            ),
