...
function handleExtendsNodes(tree, options, messages) {
	match.call(tree = applyPluginsToTree(tree, options.plugins), { tag: options.tagName }, extendsNode => {
		...
		const cleanSrc = extendsNode.attrs.src.startsWith('/') ? extendsNode.attrs.src.slice(1) : extendsNode.attrs.src
		const layoutPath = path.resolve(options.root, cleanSrc);
