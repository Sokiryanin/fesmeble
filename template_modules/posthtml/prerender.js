	...
	return {
		...
		transformIndexHtml: {
			...
			handler: async (html, { filename, server }) => {
				...
				const root = pluginOptions.root ? pluginOptions.root : srcRoot
				const plugins = []
				if (pluginOptions.extend) {
					plugins.push(posthtmlExtend({ root, ...pluginOptions.extend }))
				}
				if (pluginOptions.include) {
					plugins.push(posthtmlInclude({ root, ...pluginOptions.include }))
				}
				if (pluginOptions.fetch) {
					plugins.push(posthtmlFetch({ root, ...pluginOptions.fetch }))
				}
				if (pluginOptions.expressions) {
					plugins.push(posthtmExpressions({ root, ...pluginOptions.expressions }))
				}
