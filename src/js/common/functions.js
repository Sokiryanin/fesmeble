...
// Кешуємо висоту хедера у scroll-стані щоб уникнути повторного forced layout
let _cachedScrollHeaderHeight = null;
	...
	if (targetBlockElement) {
		...
		if (noHeader) {
			...
			if (!headerElement.classList.contains('--header-scroll')) {
				if (_cachedScrollHeaderHeight === null) {
					headerElement.style.cssText = `transition-duration: 0s;`;
					headerElement.classList.add('--header-scroll');
					_cachedScrollHeaderHeight = headerElement.offsetHeight;
					headerElement.classList.remove('--header-scroll');
					setTimeout(() => {
						headerElement.style.cssText = ``;
					}, 0);
				}
				headerItemHeight = _cachedScrollHeaderHeight;
			} else {
